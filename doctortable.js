    ...
    .onSnapshot(function(querySnapshot) {
        querySnapshot.docChanges().forEach(function(change) {
			...
			if ( change.type === "modified") {
				...
				var year=change.doc.data().date.getFullYear();
				var tm=d.getHours();
				var row123=pdtable.insertRow(rowvalue);
				row123.style.background = "#3AFF2D";
				if(date==d.getDate() && month==d.getMonth() && year==d.getFullYear() && sft=="morning" && tm<=13 ){
				if(status==1){
				row123.innerHTML+="<tr style=background:#8AFF86;><td>"+rowvalue+"</td><td>"+change.doc.data().Name+"</td><td>"+change.doc.data().Mobile+"</td><td>Success</td></tr>";
